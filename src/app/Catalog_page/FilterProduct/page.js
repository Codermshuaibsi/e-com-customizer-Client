...
export default function ProductPage() {
  ...
  // Wishlist Function
  async function AddToWishlist(i) {
    const token = localStorage.getItem("user_token");
    const res = await fetch(
      `https://ecomm-backend-7g4k.onrender.com/api/v1/addToWishlist/${i}`,
      {
        method: "POST",
        headers: {
            ...
            {loading ? (
              ...
              <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {currentProducts.map((item, idx) => (
                 ...
                 <Link href={`/order/${item._id}`}>     <button className="flex-1 bg-[#3559C7] text-white font-bold text-sm py-3 px-4 flex items-center justify-center gap-2 hover:bg-blue-800 transition">
                        <LiaShoppingBagSolid />
                        SHOP NOW
                      </button> </Link>
                      <button
